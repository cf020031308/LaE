# NOTE: This version removes code calling DGL because
# we have an environmental issue when upgrading it
import os
import time
import json
import random
...
get_score = lambda y_true, y_pred: float((
    roc_auc_score if args.dataset == 'genius' else f1_score
)(y_true.cpu(), y_pred.cpu(), average='micro'))
...
class LINKX(nn.Module):
    def __init__(self, din, dout, hidden, n_layers, dropout=0, **kw):
        super(self.__class__, self).__init__()
        self.mlpA = gnn.MLP([kw['num_nodes'], hidden])
        self.mlpX = gnn.MLP([din, hidden])
        self.W = nn.Linear(hidden * 2, hidden)
        self.mlp_final = gnn.MLP(
            in_channels=hidden, hidden_channels=hidden, out_channels=dout,
    ...
    def forward(self, x, edge_index):
        ...
        hA = self.mlpA(A)
        hX = self.mlpX(x)
        h = self.W(torch.cat((hA, hX), axis=-1))
        h = F.relu(h + hA + hX)
        h = self.mlp_final(h)
        return h
...
class ACMConv(nn.Module):
    def __init__(self, din, dout, dropout, **kw):
        super(self.__class__, self).__init__()
        self.w = nn.Linear(din, 3 * dout, bias=False)
        self.w1 = nn.Linear(dout, 1, bias=False)
        self.w2 = nn.Linear(dout, 1, bias=False)
        self.w3 = nn.Linear(dout, 1, bias=False)
        self.mix = nn.Linear(3, 3, bias=False)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        h = self.w(self.dropout(x))
        d = h.shape[-1] // 3
        h12 = A @ h[:, :-d]
        h1 = F.relu(h12[:, :d])
        h2 = F.relu(h[:, d:-d] - h12[:, d:])
        h3 = F.relu(h[:, -d:])
        alpha = torch.sigmoid(torch.cat((
            self.w1(h1), self.w2(h2), self.w3(h3)), dim=1))
        alpha = torch.softmax(self.mix(alpha / 3), dim=1)
        h = alpha[:, 0:1] * h1 + alpha[:, 1:2] * h2 + alpha[:, 2:] * h3
        return h
...
class ACMGCN(nn.Module):
    def __init__(self, din, dout, hidden, n_layers, dropout=0, **kw):
        super(self.__class__, self).__init__()
        self.gc = (
            ACMConv(din, dout, dropout) if n_layers == 1
            else nn.Sequential(
                ACMConv(din, hidden, dropout),
                *[
                    nn.Sequential(
                        nn.ReLU(),
                        ACMConv(hidden, hidden, dropout))
                    for _ in range(n_layers - 2)],
                nn.Sequential(
                    nn.ReLU(),
                    ACMConv(hidden, dout, dropout))))
...
def load_data(name):
    ...
    if args.dataset == 'weekday':
        ...
        elif args.dataset.startswith('het5chain-'):
            labels = list(range(5)) * lx
            labels = torch.tensor([
                labels[i:i+lx] for i in range(5)], dtype=torch.long)
            labels = torch.cat((labels, labels + 5))
            labels = labels.view(c, 1, lx).repeat(1, n, 1)
        elif args.dataset.startswith('het10chain-'):
            labels = list(range(10)) * lx
            labels = torch.tensor([
                labels[i:i+lx] for i in range(10)], dtype=torch.long)
            labels = labels.view(c, 1, lx).repeat(1, n, 1)
    ...
    elif args.dataset == 'BGP':
        dn = 'dataset/Bgp/'
        X = torch.from_numpy(numpy.load(dn + 'x.npy')).float()
        Y = torch.from_numpy(numpy.load(dn + 'y.npy')).long()
        E = torch.from_numpy(numpy.load(dn + 'edge_index.npy')).long()
        train_masks = []
        valid_masks = []
        test_masks = []
        masked_index = torch.arange(Y.shape[0])[Y != -1]
        masked_node_num = len(masked_index)
        for run in range(args.runs):
            random.seed(run + 1)
            shuffle_criterion = list(range(masked_node_num))
            random.shuffle(shuffle_criterion)
            train_end_index = int(0.48 * masked_node_num)
            val_end_index = train_end_index + int(0.32 * masked_node_num)
            ...
            valid_masks.append(val_mask)
            test_masks.append(test_mask)
        is_bidir = False
    elif args.dataset == 'genius':
        import scipy.io
        g = scipy.io.loadmat('dataset/genius.mat')
        E = torch.tensor(g['edge_index'], dtype=torch.long)
        X = torch.tensor(g['node_feat'], dtype=torch.float)
        Y = torch.tensor(g['label'], dtype=torch.long).squeeze()
        train_masks = []
        valid_masks = []
        test_masks = []
        for sp in numpy.load(
                'dataset/split/genius.npy', allow_pickle=True):
            sp['train']
            train_mask = torch.zeros(Y.shape[0], dtype=torch.bool)
            train_mask[sp['train']] = True
            val_mask = torch.zeros(Y.shape[0], dtype=torch.bool)
            val_mask[sp['valid']] = True
            test_mask = torch.zeros(Y.shape[0], dtype=torch.bool)
            test_mask[sp['test']] = True
            train_masks.append(train_mask)
            valid_masks.append(val_mask)
            test_masks.append(test_mask)
        if args.runs > 5:
            train_masks = (train_masks * (1 + args.runs // 5))[:args.runs]
            valid_masks = (valid_masks * (1 + args.runs // 5))[:args.runs]
            test_masks = (test_masks * (1 + args.runs // 5))[:args.runs]
        is_bidir = False
...
X, Y = map(gpu, [X, Y])
if args.method in ('SGC', 'IGNN', 'ACM'):
...
for run in range(args.runs):
    ...
    if args.method == 'IGNN':
        ...
        if args.method in ('MLP', 'SGC'):
        ...
        elif args.method == 'LIGCN':
            from ignn import LIGNN
            if args.n_layers:
                backbone = gnn.GCN(
                    in_feats, args.hidden, args.n_layers,
                    args.hidden, args.dropout)
            else:
                backbone = gpu(gnn.MLP(
                    [in_feats] + [args.hidden] * 2,
                    dropout=args.dropout))
                fwd = backbone.forward
                backbone.forward = lambda x, E: fwd(x)
            state = F.one_hot(Y, n_labels).float()
            state[~train_mask] = 0
            net = gpu(LIGNN(backbone, args.hidden, args.dropout))
            _fwd = net.forward
            net.forward = lambda x, E: _fwd(x, E, state)
        else:
            if args.method == 'JKNet':
                ...
                net = LINKX(
                    in_feats, out_feats,
                    num_nodes=X.shape[0], **args.__dict__)
            elif args.method == 'ACM':
                net = ACMGCN(in_feats, out_feats, **args.__dict__)
            elif args.method == 'DMP':
                from dmp_conv import DMPConv
                net = DMPConv(
                    in_feats, out_feats, lambda_=1, **args.__dict__)
                ...
                if args.n_layers > 1:
                    backbone = gnn.GCN(
                        in_feats, args.hidden, args.n_layers - 1,
                        args.hidden, args.dropout, bias=False)
                    net = IGNN(backbone, args.hidden, out_feats)
                    ...
                    net = IGNN(lambda x, e: x, in_feats, out_feats)
            else:
                net = {
        ...
        # input_label = 0.5 if args.input_label == 1 else args.input_label
        ...
        for epoch in range(1, 1 + args.max_epochs):
            ...
            #     pred = labelize(state)
            #     intra_rate = (
            #         (pred[E[0]] == pred[E[1]]).sum().float()
            #         / E.shape[1] / (n_labels if is_multiclass else 1)
            #     ).item()
            #     input_label = 1 - intra_rate
            #     # print(epoch, input_label)
        ...
        if args.for_iter:
            ...
            # print('forward iteration:', sum(fwd_counters[:ev.best_epoch]))
            # print('forward time:', sum(fwd_times[:ev.best_epoch]))
            # print('forward contract:', fwd_contracts[ev.best_epoch - 1])
        if args.back_iter:
            print('backward iterations:', bkd_counters)
            print('backward contracts:', bkd_contracts)
            print('backward contract:', bkd_contracts[traini])
            # print('backward times:', bkd_times)
            # print('backward iteration:', sum(bkd_counters[:ev.best_epoch]))
            # print('backward time:', sum(bkd_times[:ev.best_epoch]))
            # print('backward contract:', bkd_contracts[ev.best_epoch - 1])
